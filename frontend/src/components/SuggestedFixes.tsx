  ...
  if (suggestions.length === 0) {
    return (
      <div className="py-8 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-xs text-slate-500 max-w-[200px] font-bold uppercase tracking-widest leading-relaxed">
          Select a node to generate <span className="text-indigo-400">Architect Advice</span>
        </p>
      </div>
    );
  }
      ...
      {suggestions.map((suggestion, index) => (
        ...
        >
          <div className="flex items-start gap-3">
            <FileCode className="w-4 h-4 text-indigo-400 mt-1 flex-shrink-0 group-hover:scale-110 transition-transform" />
